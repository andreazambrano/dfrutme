...
export class DataApiService {
  ...
  constructor(
	...
	saveTixFree(tix :TixInterface){
  	...
  	getSalePending(){	
		const url_api='https://db.buckapi.com:3025/api/sale?filter[where][status]=new';
		return (this.sales = this.http.get(url_api));
	}
	getOrderPending(){	
		const url_api='https://db.buckapi.com:3025/api/order?filter[where][status]=new';
	...
	getBulbos(){
		const url_api = 'https://db.buckapi.com:3025/api/tixes?filter[where][and][0][status]=activated&filter[where][and][1][category]=Bulbos';
		return this.http.get(url_api);
	}
	getFrutos(){
		const url_api = 'https://db.buckapi.com:3025/api/tixes?filter[where][and][0][status]=activated&filter[where][and][1][category]=Frutos';
		return this.http.get(url_api);
	}
	getLegumbres(){
		const url_api = 'https://db.buckapi.com:3025/api/tixes?filter[where][and][0][status]=activated&filter[where][and][1][category]=Legumbres';
		return this.http.get(url_api);
	}
	getTuberculos(){
		const url_api = 'https://db.buckapi.com:3025/api/tixes?filter[where][and][0][status]=activated&filter[where][and][1][category]=Tubérculos';
		return this.http.get(url_api);
	}
	getVerduras(){
		const url_api = 'https://db.buckapi.com:3025/api/tixes?filter[where][and][0][status]=activated&filter[where][and][1][category]=Verduras';
		return this.http.get(url_api);
	}
	getInfo(){
	...
	getSaleById(id:string){
		let indice = id;
		const url_api=`https://db.buckapi.com:3025/api/sale/${indice}`;
